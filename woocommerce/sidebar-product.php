  ...
  <form class="bt-product-filter-form" action="" method="get">
    ...
    <div class="bt-product-filter-fields">
      ...
      ?>
            ...
            case 'comfort_scale':
              $field_name = __('Comfort Scale', 'somnia');
              $field_value = (isset($_GET['comfort_scale'])) ? $_GET['comfort_scale'] : '';
              somnia_product_field_multiple_html('comfort_scale', $field_name, $field_value);
              break;

            case 'mattress_type':
              $field_name = __('Mattress Type', 'somnia');
              $field_value = (isset($_GET['mattress_type'])) ? $_GET['mattress_type'] : '';
              somnia_product_field_multiple_html('mattress_type', $field_name, $field_value);
              break;
        ...
        <?php
            ...
            default:
              // Handle WooCommerce attributes selected from ACF (taxonomy names like pa_xxx)
              if (strpos($filter_type, 'pa_') === 0 && taxonomy_exists($filter_type)) {
                // Check if this attribute is enabled in search
                if (function_exists('somnia_is_attribute_enabled_in_search') && somnia_is_attribute_enabled_in_search($filter_type)) {
                  $attribute_type = function_exists('somnia_get_attribute_type') ? somnia_get_attribute_type($filter_type) : 'select';
                  $field_name = function_exists('wc_attribute_label') ? wc_attribute_label($filter_type) : '';
                  $field_value = (isset($_GET[$filter_type])) ? $_GET[$filter_type] : '';

                  // Check if it's a color attribute
                  if ($attribute_type === 'color') {
                    somnia_product_field_multiple_color_html($filter_type, $field_name, $field_value);
                  } else {
                    somnia_product_field_multiple_html($filter_type, $field_name, $field_value);
                  }
                }
              }
              break;
      ...
      <?php
        ...
        // Display all enabled WooCommerce attributes (non-color)
        ...
        if (!empty($enabled_attributes)) {
          foreach ($enabled_attributes as $attr_data) {
            ...
            $field_name = function_exists('wc_attribute_label') ? wc_attribute_label($taxonomy) : '';
            $field_value = (isset($_GET[$taxonomy])) ? $_GET[$taxonomy] : '';

            if ($attribute_type === 'image') {
              somnia_product_field_multiple_html($taxonomy, $field_name, $field_value);
            } else {
              somnia_product_field_multiple_html($taxonomy, $field_name, $field_value);
            }
          }
        ...
        if (!empty($color_taxonomies)) {
          foreach ($color_taxonomies as $color_taxonomy) {
            $field_name = function_exists('wc_attribute_label') ? wc_attribute_label($color_taxonomy) : __('Colors', 'somnia');
